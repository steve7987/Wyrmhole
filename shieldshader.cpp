...
bool ShieldShader::Initialize(ID3D11Device* device, HWND hwnd){
	
	if (!InitializeShader(device, hwnd, SHIELDSHADERFILEVS, SHIELDSHADERFILEPS)){
		return false;
	}
...
void ShieldShader::Shutdown(){
	ShutdownShader();
}

bool ShieldShader::Render(ID3D11DeviceContext* deviceContext, int indexCount, D3DXMATRIX worldMatrix, 
...
bool ShieldShader::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename){
	HRESULT result;
	...
	D3D11_BUFFER_DESC shieldBufferDesc;  //new from texture shader
	...
	result = D3DX11CompileFromFile(vsFilename, NULL, NULL, "ShieldVertexShader", "vs_4_0", 
	...
	result = D3DX11CompileFromFile(psFilename, NULL, NULL, "ShieldPixelShader", "ps_4_0",
	...
	shieldBufferDesc.Usage = D3D11_USAGE_DYNAMIC;
	shieldBufferDesc.ByteWidth = sizeof(ShieldBufferType);
	shieldBufferDesc.BindFlags = D3D11_BIND_CONSTANT_BUFFER;
	shieldBufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	shieldBufferDesc.MiscFlags = 0;
	shieldBufferDesc.StructureByteStride = 0;
	
	// Create the constant buffer pointer so we can access the vertex shader constant buffer from within this class.
	result = device->CreateBuffer(&shieldBufferDesc, NULL, &m_shieldBuffer);
	if(FAILED(result))
	{
		return false;
	}
...
void ShieldShader::OutputShaderErrorMessage(ID3D10Blob* errorMessage, HWND hwnd, WCHAR* shaderFilename){
...
bool ShieldShader::SetShaderParameters(ID3D11DeviceContext* deviceContext, D3DXMATRIX worldMatrix, 
					   D3DXMATRIX viewMatrix, D3DXMATRIX projectionMatrix, ID3D11ShaderResourceView* texture, 
					   D3DXVECTOR3 direction, float strength)
{
	HRESULT result;
	...
	result = deviceContext->Map(m_shieldBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource);
	if(FAILED(result))
	{
		return false;
	}
	...
	// Get a pointer to the data in the constant buffer.
	dataPtr2 = (ShieldBufferType*)mappedResource.pData;
	...
	// Unlock the constant buffer.
	deviceContext->Unmap(m_shieldBuffer, 0);

	// Set the position of the light constant buffer in the pixel shader.
	bufferNumber = 0;

	// Finally set the light constant buffer in the pixel shader with the updated values.
	deviceContext->PSSetConstantBuffers(bufferNumber, 1, &m_shieldBuffer);

	return true;
}

//set our data layout to active and then set the shaders to be active
void ShieldShader::RenderShader(ID3D11DeviceContext* deviceContext, int indexCount)
{
