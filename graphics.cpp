...
bool Graphics::RenderObjectSwitch(Renderable * m, int shaderType, float * parameters){
	...
	case SHADER_TUBE:
		...
		ret = RenderObjectTube(m, parameters[0], D3DXVECTOR3(parameters[1], parameters[2], parameters[3]), D3DXVECTOR3(parameters[4], parameters[5], parameters[6]),
							   ...
							   D3DXVECTOR3(parameters[19], parameters[20], parameters[21]), parameters[22], parameters[23], parameters[24], parameters[25], parameters[26]);
		delete parameters;
		return ret;
		break;
	default:
						   ...
						   D3DXVECTOR3 Center, D3DXVECTOR3 v1, D3DXVECTOR3 v2, float s1, float s2, float s3, float maxAngle, float radius)
{
	...
	bool result = m_TubeShader->Render(m_d3d->GetDeviceContext(), m->GetIndexCount(), m->GetWorldMatrix(), 
										...
										p1, p2, p3, p4, Center, v1, v2, s1, s2, s3, maxAngle, radius);
	if (!result){
		return false;
	}
	return true;
